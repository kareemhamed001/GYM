    ...
    <div class="row my-2">
        ...
        <nav class="breadcrumb-style-one mb-3  col-lg-6" aria-label="breadcrumb">
            <ol class="breadcrumb">
                ...
                <li class="breadcrumb-item"><a @if($user->role_as==1) href="{{url('coach/coaches')}}"  @elseif($user->role_as==2) href="{{url('coach/users')}}" @endif >@if($user->role_as==1) Coaches  @elseif($user->role_as==2) Users @endif</a></li>
                <li class="breadcrumb-item active" aria-current="page">Create user</li>
            </ol>
        </nav>
        <h4 class="my-3">Edit @if($user->role_as==1) Coach  @elseif($user->role_as==2) User @endif</h4>

        <form id="adduserForm" class="my-2 row">
            @csrf
            <div class="col-12 d-flex justify-content-center">
                <label for="coverImage">
                    <img id="preview" class="img-fluid col-6 rounded-circle border border-white" style="width: 18rem ;height: 18rem;object-fit: scale-down" src="{{asset($user->profile_image)}}" alt="">
                </label>

            </div>
            <input type="hidden" name="coach_id" value="{{Auth::user()?->id}}">
            <input type="hidden" name="role_as" value="{{$user->role_as}}">
            ...
            <div class="form-group col-md-6 my-2 px-1">
                ...
                <input type="text" name="address" id="address" class="form-control" placeholder="Enter user address *"  value="{{$user->address}}">
                @error('address')
                <span class="text-danger">{{$message}}</span>
                @enderror
            </div>
            ...
            <div class="flex-wrap p-0 d-flex"  id="coachInfo">
                @if($user->role_as==1)
